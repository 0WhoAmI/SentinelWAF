...
namespace SentinelWaf.Infrastructure.Detectors.Regex
{
    public class RegexAdvancedDetector : IAttackDetector
    {
        ...
        private static readonly SystemRegex.Regex _sqliRegex =
            new SystemRegex.Regex(
                @"(?i)(?:['""]|%22|%27|%60|%2527)\s*(?:and|or|\|\||&&)\s*(?:['""]|%22|%27|%60|%2527)?\s*\d+\s*(?:=|=>|<=|<|>)\s*(?:['""]|%22|%27|%60|%2527)?\s*\d+|(?:\b(?:union\s+(?:all\s+)?select|waitfor\s+delay|xp_cmdshell|sleep\s*\()\b)|(?:;\s*exec\s+)",
                SystemRegex.RegexOptions.Compiled, RegexTimeout);

        private static readonly SystemRegex.Regex _xssRegex =
            new SystemRegex.Regex(
                @"(?i)(?:<\w+[^>]*?[\s\/]+(?:on[a-z]+|xmlns)\s*=)|(?:(?:javascript|vbscript|data):.*?(?:\%250A|alert|prompt|confirm))",
                SystemRegex.RegexOptions.Compiled, RegexTimeout);
